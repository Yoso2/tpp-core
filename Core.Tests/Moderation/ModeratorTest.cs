...
namespace Core.Tests.Moderation
{
    ...
    public class ModeratorTest
    {
        ...
        public async Task timeout_after_too_many_points()
        {
            ...
            Assert.IsTrue(await moderator.Check(new Message(user, msg1, MessageSource.Chat)));
            executor.Verify(invocation, Times.Never);

            const string msg2 = "enough points for timeout";
            Assert.IsFalse(await moderator.Check(new Message(user, msg2, MessageSource.Chat)));
            executor.Verify(invocation, Times.Once);

            const string msg3 = "points reset after timeout, no additional timeout yet";
            Assert.IsTrue(await moderator.Check(new Message(user, msg3, MessageSource.Chat)));
            executor.Verify(invocation, Times.Once);

            const string msg4 = "timeout again after points were reached a second time";
            Assert.IsFalse(await moderator.Check(new Message(user, msg4, MessageSource.Chat)));
        ...
        public async Task points_decay_over_time()
        {
            ...
            Assert.IsTrue(await moderator.Check(new Message(user, msg2, MessageSource.Chat)));
        ...
        public async Task points_below_minimum_dont_count()
        {
            ...
            Assert.IsTrue(await moderator.Check(new Message(user, "no points", MessageSource.Chat)));
            Assert.IsTrue(await moderator.Check(new Message(user, "still no points", MessageSource.Chat)));
            Assert.IsTrue(await moderator.Check(new Message(user, "everything is fine", MessageSource.Chat)));
        }
    }
}
